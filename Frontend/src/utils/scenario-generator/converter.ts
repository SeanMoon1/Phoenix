...
import type { ScenarioGeneratorEvent, ChoiceOption, ConversionOptions } from '../../types';
...
export class ScenarioConverter {
  ...
  private generateOptionSQL(
    ...
    const pointsSpeed = option.speedPoints || 0;
    const pointsAccuracy = option.accuracyPoints || 0;
    const expReward = option.expPoints || 0;
    const isCorrect = pointsSpeed > 0 && pointsAccuracy > 0 ? 1 : 0;
    const nextEventId = option.nextEventId ? `'${option.nextEventId}'` : 'NULL';
    const answerPreview =
      option.choiceText.substring(0, 30) + (option.choiceText.length > 30 ? '...' : '');
    ...
    return sqlTemplates.option
      ...
      .replace('{{answer}}', this.escapeSQL(option.choiceText))
      .replace('{{reaction}}', this.escapeSQL(option.reactionText || ''))
  ...
  generateStatistics(data: ScenarioGeneratorEvent[]): {
    ...
    const disasterTypes = [
      ...new Set(data.map(event => event.disasterType).filter((type): type is string => Boolean(type))),
    ];
    const difficulties = [
      ...new Set(data.map(event => event.difficulty).filter((diff): diff is string => Boolean(diff))),
    ];
    const riskLevels = [
      ...new Set(data.map(event => event.riskLevel).filter((level): level is string => Boolean(level))),
    ];
